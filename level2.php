...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $stmt = $conn->prepare('INSERT INTO level1_comments (roll_no, level1_comments, level2_comments, verification_status) 
                                VALUES (?, ?, ?, ?) 
        ...
        foreach ($roll_nos as $roll_no) {
            ...
            $comment = isset($level2_comments[$roll_no]) ? $level2_comments[$roll_no] : '';
            $status = isset($verification_status[$roll_no]) ? $verification_status[$roll_no] : 'Not Verified';

            // Bind parameters and execute statement
            $stmt->bind_param("ssss", $roll_no, $prev_level1_comment, $comment, $status);
            if (!$stmt->execute()) {
...
$query = "SELECT roll_no, level1_comments FROM students";
                    ...
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    <textarea name="level2_comments[<?php echo htmlspecialchars($row['roll_no']); ?>]"></textarea>
                                </td>
                                <td>
